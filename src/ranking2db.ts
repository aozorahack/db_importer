...
function process_response($: CheerioStatic,
                          year_month: string,
                          book_ids: number[]) {
  return $('tr[valign]').map((idx: number, tr: CheerioElement) => {
    if (idx === 0) {
      return null;
    }

    // const rank = $(tr).find('td:nth-child(1)').text();
    const book_id = parseInt(re.exec($(tr).find('td:nth-child(2) a')
                                     .attr('href'))[1], 10);
    ...
    const entry = {year_month, book_id,
                   access : parseInt($(tr).find('td:nth-child(4)').text(), 10)};
...
async function import_to_db(db: IDB) {
  const base_options = {
    headers: {
  ...
  const plist = entries.map(async (e: Entry) => {
    ...
    try {
      ...
      await db.replace_one('ranking_etags',
                           {year_month_type},
